...
const ALLOW_SELF_CHAT = String(process.env.ALLOW_SELF_CHAT || "true").toLowerCase() !== "false";
const WA_MESSAGE_DEBUG = String(process.env.WA_MESSAGE_DEBUG || "false").toLowerCase() === "true";
...
const DEFAULT_PAPER_BALANCE = Math.max(100, Number(process.env.DEFAULT_PAPER_BALANCE || 10000));
const PAPER_TRADE_FEE_BPS = Math.max(0, Math.min(100, Number(process.env.PAPER_TRADE_FEE_BPS || 10)));
const BACKTEST_CANDLE_LIMIT = Math.max(120, Math.min(1000, Number(process.env.BACKTEST_CANDLE_LIMIT || 420)));
...
const WHATSAPP_AUTH_DIR = path.resolve(process.env.WHATSAPP_AUTH_DIR || path.join(__dirname, "session"));
const WA_LOG_LEVEL = process.env.WA_LOG_LEVEL || "silent";
...
const STRATEGIES = [
    { id: "composite", name: "Composite AI Rules", style: "gabungan trend, pullback, dan breakout" },
    { id: "trend", name: "Trend Rider", style: "ikut arah EMA dan MACD" },
    { id: "pullback", name: "Smart Pullback", style: "cari pantulan sehat di tren naik" },
    { id: "breakout", name: "Volume Breakout", style: "konfirmasi tembus resistance dengan volume" }
];
...
const memoryCache = {
    ...
    botMessageFingerprints: new Map()
};
...
function createDefaultState() {
    return {
        ...
        paperHistory: {},
        strategySettings: {},
        settings: { defaultMode: DEFAULT_MODE }
    };
}
...
function loadState() {
    try {
        ...
        return {
            ...
            paperAccounts: parsed.paperAccounts || {},
            paperHistory: parsed.paperHistory || {},
            strategySettings: parsed.strategySettings || {},
...
function isClosedSignalSessionError(err) {
    ...
    return message.includes("decrypted message with closed session") ||
        message.includes("message with closed session");
}
...
function pruneBotMessageTracking() {
    ...
    for (const [key, at] of memoryCache.botMessageIds) {
        if (at < cutoff) memoryCache.botMessageIds.delete(key);
    }
    for (const [key, at] of memoryCache.botMessageFingerprints) {
        if (at < cutoff) memoryCache.botMessageFingerprints.delete(key);
    }
}
...
function markBotOutgoingMessage(jid, content, result) {
    ...
    if (id) memoryCache.botMessageIds.set(id, Date.now());
    if (fingerprint) memoryCache.botMessageFingerprints.set(fingerprint, Date.now());
...
function wasSentByBot(msg, text) {
    ...
    return Boolean(
        (id && memoryCache.botMessageIds.has(id)) ||
        (fingerprint && memoryCache.botMessageFingerprints.has(fingerprint))
    );
}

function isOwnerSelfChat(msg) {
    if (!ALLOW_SELF_CHAT || !msg?.key?.fromMe) return false;
    const candidates = [msg.key.remoteJid, msg.key.remoteJidAlt, msg.key.participant, msg.key.participantAlt]
        .filter(Boolean);
    return candidates.includes(OWNER_JID) || candidates.some(jid => {
        const phone = String(jid).split("@")[0].split(":")[0].replace(/\D/g, "");
        return phone && phone === WHATSAPP_PHONE_NUMBER;
    });
}
...
function normalizeStrategyId(input) {
    ...
    if (!text) return "composite";
    if (/\b(all|multi|gabungan|komposit|composite)\b/.test(text)) return "composite";
    if (/\b(trend|momentum|rider|ema)\b/.test(text)) return "trend";
    if (/\b(pullback|pantul|reversal|mean)\b/.test(text)) return "pullback";
    if (/\b(breakout|break|volume|tembus)\b/.test(text)) return "breakout";
    return STRATEGIES.find(strategy => strategy.id === text)?.id || "composite";
}

function strategyInfo(strategyId) {
    return STRATEGIES.find(strategy => strategy.id === normalizeStrategyId(strategyId)) || STRATEGIES[0];
}

function strategyActionLabel(action) {
    if (action === "ENTRY") return "ENTRY";
    if (action === "EXIT") return "EXIT/SELL";
    return "WAIT";
}

function candleVolumeRatio(candles, length = 20) {
    const volumes = candles.slice(-length - 1, -1).map(candle => candle.volume).filter(value => value > 0);
    const latest = Number(candles[candles.length - 1]?.volume || 0);
    if (volumes.length < Math.min(8, length) || latest <= 0) return 1;
    const average = volumes.reduce((sum, value) => sum + value, 0) / volumes.length;
    return average > 0 ? latest / average : 1;
}

function evaluateSingleStrategy(candles, strategyId = "composite") {
    const id = normalizeStrategyId(strategyId);
    if (id === "composite") return evaluateCompositeStrategy(candles);

    if (!Array.isArray(candles) || candles.length < 60) {
        return {
            id,
            name: strategyInfo(id).name,
            action: "WAIT",
            score: 0,
            entryScore: 0,
            exitScore: 0,
            reasons: ["data candle belum cukup"],
            indicators: {}
        };
    }
    ...
    const sr = supportResistance(priorCandles.length >= 20 ? priorCandles : candles, Math.min(60, Math.max(20, candles.length - 1)));
    const atrValue = atr(candles, 14) || last * 0.01;
    const atrPct = (atrValue / last) * 100;
    const volumeRatio = candleVolumeRatio(candles, 20);
    const greenCandle = Number(lastCandle.close) > Number(lastCandle.open);
    const movePct = previous ? ((last - previous) / previous) * 100 : 0;
    const distanceToEma21 = ema21 ? Math.abs((last - ema21) / last) * 100 : 99;
    const distanceFromSupport = ((last - sr.support) / last) * 100;
    const closeAbove = value => Number.isFinite(value) && last > value;
    const closeBelow = value => Number.isFinite(value) && last < value;
    ...
    if (id === "trend") {
        if (ema9 && ema21 && ema9 > ema21) {
            entryScore += 26;
            entryReasons.push("EMA 9 > EMA 21");
        } else {
            exitScore += 28;
            exitReasons.push("EMA pendek melemah");
        }
        if (closeAbove(ema50)) {
            entryScore += 22;
            entryReasons.push("harga di atas EMA 50");
        } else {
            exitScore += 24;
            exitReasons.push("harga di bawah EMA 50");
        }
        if (macdData.histogram != null && macdData.histogram > 0) {
            entryScore += 18;
            entryReasons.push("MACD positif");
        } else if (macdData.histogram != null) {
            exitScore += 16;
            exitReasons.push("MACD negatif");
        }
        if (rsi14 != null && rsi14 >= 45 && rsi14 <= 68) {
            entryScore += 16;
            entryReasons.push(`RSI trend sehat ${rsi14.toFixed(1)}`);
        }
        if (rsi14 != null && rsi14 > 74) {
            exitScore += 18;
            exitReasons.push(`RSI terlalu panas ${rsi14.toFixed(1)}`);
        }
        if (volumeRatio > 1.1 && movePct > 0) {
            entryScore += 10;
            entryReasons.push(`volume konfirmasi ${volumeRatio.toFixed(1)}x`);
        }
        if (movePct < -1.2) {
            exitScore += 10;
            exitReasons.push(`candle terakhir turun ${movePct.toFixed(2)}%`);
        }
    }

    if (id === "pullback") {
        const trendUp = closeAbove(ema50) && ema21 && ema50 && ema21 > ema50;
        if (trendUp) {
            entryScore += 25;
            entryReasons.push("tren besar masih naik");
        } else {
            exitScore += 24;
            exitReasons.push("tren besar belum mendukung");
        }
        if (rsi14 != null && rsi14 >= 34 && rsi14 <= 54) {
            entryScore += 24;
            entryReasons.push(`RSI pullback sehat ${rsi14.toFixed(1)}`);
        }
        if (distanceToEma21 <= 1.4) {
            entryScore += 20;
            entryReasons.push("harga dekat EMA 21");
        }
        if (distanceFromSupport >= 0 && distanceFromSupport <= 2.0) {
            entryScore += 14;
            entryReasons.push("harga dekat support");
        }
        if (greenCandle) {
            entryScore += 10;
            entryReasons.push("candle mulai memantul");
        }
        if (closeBelow(ema50)) {
            exitScore += 30;
            exitReasons.push("harga kehilangan EMA 50");
        }
        if (ema9 && ema21 && ema9 < ema21) {
            exitScore += 18;
            exitReasons.push("EMA 9 di bawah EMA 21");
        }
        if (rsi14 != null && rsi14 > 68) {
            exitScore += 14;
            exitReasons.push("pullback sudah tidak murah");
        }
    }

    if (id === "breakout") {
        const breakoutLevel = sr.resistance;
        if (last > breakoutLevel * 1.001) {
            entryScore += 34;
            entryReasons.push("close tembus resistance");
        } else if (last < breakoutLevel * 0.995) {
            exitScore += 18;
            exitReasons.push("belum mampu tembus resistance");
        }
        if (volumeRatio > 1.2) {
            entryScore += 24;
            entryReasons.push(`volume breakout ${volumeRatio.toFixed(1)}x`);
        }
        if (closeAbove(ema21)) {
            entryScore += 16;
            entryReasons.push("harga di atas EMA 21");
        } else {
            exitScore += 20;
            exitReasons.push("harga di bawah EMA 21");
        }
        if (macdData.histogram != null && macdData.histogram > 0) {
            entryScore += 14;
            entryReasons.push("momentum MACD mendukung");
        }
        if (rsi14 != null && rsi14 >= 50 && rsi14 <= 72) {
            entryScore += 12;
            entryReasons.push(`RSI breakout valid ${rsi14.toFixed(1)}`);
        }
        if (rsi14 != null && rsi14 > 78) {
            exitScore += 22;
            exitReasons.push("breakout rawan blow-off");
        }
        if (greenCandle && movePct > atrPct * 0.35) {
            entryScore += 10;
            entryReasons.push("range candle menguat");
        }
    }

    const action = entryScore >= 62 && entryScore >= exitScore + 8
        ? "ENTRY"
        : exitScore >= 58 && exitScore > entryScore
            ? "EXIT"
            : "WAIT";
    const score = Math.max(entryScore, exitScore);
    const reasons = action === "EXIT" ? exitReasons : action === "ENTRY" ? entryReasons : [...entryReasons, ...exitReasons];
    ...
    return {
        ...
        exitScore: Math.min(100, Math.round(exitScore)),
        reasons: reasons.slice(0, 4).length ? reasons.slice(0, 4) : ["belum ada edge strategi yang jelas"],
        indicators: {
            rsi: rsi14,
            ema9,
            ema21,
...
function evaluateCompositeStrategy(candles) {
    const parts = ["trend", "pullback", "breakout"].map(id => evaluateSingleStrategy(candles, id));
    const entryScore = Math.round(parts.reduce((sum, part) => sum + part.entryScore, 0) / parts.length);
    const exitScore = Math.round(parts.reduce((sum, part) => sum + part.exitScore, 0) / parts.length);
    const action = entryScore >= 62 && entryScore >= exitScore + 8
        ? "ENTRY"
        : exitScore >= 58 && exitScore > entryScore
            ? "EXIT"
            : "WAIT";
    const leaders = [...parts].sort((a, b) => b.score - a.score).slice(0, 2);
    return {
        id: "composite",
        name: strategyInfo("composite").name,
        action,
        score: Math.max(entryScore, exitScore),
        entryScore,
        exitScore,
        reasons: leaders.flatMap(part => part.reasons.slice(0, 1)).slice(0, 4),
        indicators: leaders[0]?.indicators || {},
        parts
    };
}

function buildStrategySignals(candles) {
    return STRATEGIES.map(strategy => evaluateSingleStrategy(candles, strategy.id));
}

function formatStrategyStack(strategies) {
    const rows = (strategies || []).filter(Boolean);
    if (!rows.length) return "Belum ada strategi aktif.";
    return rows.map(strategy => {
        const reason = strategy.reasons?.[0] || "menunggu konfirmasi";
        return `${strategy.name}: ${strategyActionLabel(strategy.action)} ${strategy.score}/100 - ${reason}`;
    }).join("\n");
}
...
async function analyzeAsset(asset, mode = DEFAULT_MODE, options = {}) {
    ...
    const strategies = buildStrategySignals(candles15m);
...
*STRATEGY STACK*
${formatStrategyStack(result.strategies)}
...
function tradeFee(value) {
    return Number(value || 0) * (PAPER_TRADE_FEE_BPS / 10000);
}

async function runBacktest(asset, strategyId = "composite", mode = DEFAULT_MODE) {
    const item = typeof asset === "string" ? normalizeAsset(asset) : asset;
    if (!item) throw new Error("Koin tidak ada di watchlist.");

    const strategy = strategyInfo(strategyId);
    const interval = mode === "investor" ? "1h" : "15m";
    const candles = (await getKlines(item.symbol, interval, BACKTEST_CANDLE_LIMIT, { force: false }))
        .filter(candle => Number.isFinite(Number(candle.close)) && Number(candle.close) > 0);
    if (candles.length < 80) throw new Error(`Data candle ${item.asset} belum cukup untuk backtest.`);

    const initialBalance = DEFAULT_PAPER_BALANCE;
    let cash = initialBalance;
    let position = null;
    let peakEquity = initialBalance;
    let maxDrawdownPct = 0;
    const trades = [];
    const warmup = Math.min(70, Math.max(55, Math.floor(candles.length * 0.25)));

    for (let i = warmup; i < candles.length; i++) {
        const slice = candles.slice(0, i + 1);
        const candle = candles[i];
        const price = Number(candle.close);
        const signal = evaluateSingleStrategy(slice, strategy.id);

        if (!position && signal.action === "ENTRY" && cash > 5) {
            const allocation = cash * 0.95;
            const fee = tradeFee(allocation);
            const quantity = (allocation - fee) / price;
            position = {
                entryPrice: price,
                quantity,
                cost: allocation,
                entryAt: candle.closeTime || candle.openTime,
                highestPrice: price,
                entrySignal: signal
            };
            cash -= allocation;
            ...
            const atrPct = clampNumber(signal.indicators?.atrPct || 1.2, 0.35, 8);
            const stopPct = clampNumber(atrPct * (mode === "investor" ? 2.2 : 1.4), mode === "investor" ? 2.2 : 1.1, mode === "investor" ? 8 : 4.5);
            const targetPct = clampNumber(atrPct * (mode === "investor" ? 4.2 : 2.6), mode === "investor" ? 4.0 : 2.0, mode === "investor" ? 16 : 8);
            const trailPct = clampNumber(atrPct * (mode === "investor" ? 2.4 : 1.7), mode === "investor" ? 2.4 : 1.2, mode === "investor" ? 9 : 5);

            let exitReason = null;
            if (price <= position.entryPrice * (1 - stopPct / 100)) exitReason = "SL volatilitas";
            if (!exitReason && price >= position.entryPrice * (1 + targetPct / 100)) exitReason = "TP volatilitas";
            if (!exitReason && position.highestPrice >= position.entryPrice * (1 + targetPct / 200) && price <= position.highestPrice * (1 - trailPct / 100)) exitReason = "trailing protect";
            if (!exitReason && signal.action === "EXIT" && gainPct > -stopPct * 0.7) exitReason = "exit signal";
            if (!exitReason && i === candles.length - 1) exitReason = "akhir data";

            if (exitReason) {
                const gross = position.quantity * price;
                const fee = tradeFee(gross);
                const net = gross - fee;
                const pnl = net - position.cost;
                cash += net;
                trades.push({
                    ...
                    pnl,
                    pnlPct: (pnl / position.cost) * 100,
                    entryAt: position.entryAt,
                    exitAt: candle.closeTime || candle.openTime,
                    reason: exitReason
                });
                position = null;
            }
    ...
    const finalEquity = cash + (position ? position.quantity * lastPrice : 0);
    const wins = trades.filter(trade => trade.pnl > 0);
    const losses = trades.filter(trade => trade.pnl < 0);
    const grossProfit = wins.reduce((sum, trade) => sum + trade.pnl, 0);
    const grossLoss = Math.abs(losses.reduce((sum, trade) => sum + trade.pnl, 0));
    const lastSignal = evaluateSingleStrategy(candles, strategy.id);
    ...
    return {
        ...
        from: candles[warmup]?.openTime || candles[0].openTime,
        to: candles[candles.length - 1].closeTime || candles[candles.length - 1].openTime,
        initialBalance,
        finalEquity,
...
function buildBacktestMessage(result) {
    const lastTrades = result.trades.slice(-3).reverse();
    const profitFactor = Number.isFinite(result.profitFactor) ? result.profitFactor.toFixed(2) : "inf";
    let text = `*BACKTEST ${result.asset}/USDT*\n`;
    text += `Strategi: ${result.strategy.name}\n`;
    text += `Mode: ${result.mode.toUpperCase()} | Candle: ${result.interval} | Sample: ${result.candles}\n`;
    text += `Periode: ${timeText(result.from)} - ${timeText(result.to)}\n\n`;
    text += `Modal awal: ${formatUsd(result.initialBalance, 2)}\n`;
    text += `Equity akhir: ${formatUsd(result.finalEquity, 2)}\n`;
    text += `PnL: *${formatSignedUsd(result.pnl, 2)}* (${formatPct(result.pnlPct)})\n`;
    text += `Trade: ${result.trades.length} | Win rate: ${result.winRate.toFixed(1)}%\n`;
    text += `Profit factor: ${profitFactor} | Max DD: ${result.maxDrawdownPct.toFixed(2)}%\n`;
    text += `Sinyal terakhir: ${strategyActionLabel(result.lastSignal.action)} ${result.lastSignal.score}/100\n`;
    if (result.openPosition) text += `Open virtual: masih ada posisi simulasi di akhir data.\n`;
    if (lastTrades.length) {
        text += `\n*3 TRADE TERAKHIR*\n`;
        for (const trade of lastTrades) {
            text += `${formatPct(trade.pnlPct)} | ${trade.reason} | exit ${formatUsd(trade.exitPrice, priceDigits(trade.exitPrice))}\n`;
        }
    }
    text += `\nBacktest ini simulasi historis, bukan jaminan performa live. Fee simulasi: ${PAPER_TRADE_FEE_BPS} bps.`;
    return text.trim();
}

async function buildStrategyBoard(asset, mode = DEFAULT_MODE) {
    const item = typeof asset === "string" ? normalizeAsset(asset) : asset;
    if (!item) throw new Error("Koin tidak ada di watchlist.");
    const ticker = await getTicker(item.symbol, { force: FORCE_REFRESH_ON_REQUEST });
    const candles = await getKlines(item.symbol, mode === "investor" ? "1h" : "15m", 150, { force: false, liveTicker: ticker });
    const strategies = buildStrategySignals(candles);
    const sorted = [...strategies].sort((a, b) => b.score - a.score);
    const digits = priceDigits(ticker.price);

    let text = `*STRATEGY LAB ${item.asset}/USDT*\n`;
    text += `Harga: ${formatUsd(ticker.price, digits)} (${formatPct(ticker.changePct)})\n`;
    text += `Mode: ${mode.toUpperCase()} | Data: ${timeText(ticker.fetchedAt)}\n\n`;
    for (const strategy of sorted) {
        text += `*${strategy.name}*\n`;
        text += `${strategyActionLabel(strategy.action)} | score ${strategy.score}/100 | entry ${strategy.entryScore}/exit ${strategy.exitScore}\n`;
        text += `${strategy.reasons.slice(0, 2).join("; ")}\n\n`;
    }
    text += `Coba: *backtest ${item.asset} trend*, *backtest ${item.asset} pullback*, atau *paper beli ${item.asset} 1000*.`;
    return text.trim();
}

function getPaperAccount(jid) {
    if (!state.paperAccounts[jid]) {
        state.paperAccounts[jid] = {
            startingBalance: DEFAULT_PAPER_BALANCE,
            balance: DEFAULT_PAPER_BALANCE,
            positions: {},
            createdAt: Date.now(),
            updatedAt: Date.now()
        };
    ...
    state.paperAccounts[jid].positions = state.paperAccounts[jid].positions || {};
    return state.paperAccounts[jid];
}

function appendPaperHistory(jid, event) {
    state.paperHistory[jid] = state.paperHistory[jid] || [];
    state.paperHistory[jid].push({ ...event, at: Date.now() });
    state.paperHistory[jid] = state.paperHistory[jid].slice(-80);
}

function resetPaperAccount(jid, balance = DEFAULT_PAPER_BALANCE) {
    const startingBalance = Math.max(100, Number(balance || DEFAULT_PAPER_BALANCE));
    state.paperAccounts[jid] = {
        startingBalance,
    ...
    state.paperHistory[jid] = [];
    saveState();
    return state.paperAccounts[jid];
}

async function paperBuy(jid, asset, amount = null, mode = DEFAULT_MODE) {
    const account = getPaperAccount(jid);
    const item = typeof asset === "string" ? normalizeAsset(asset) : asset;
    if (!item) throw new Error("Koin tidak ada di watchlist.");
    const desired = amount || account.balance * 0.1;
    if (desired > account.balance) throw new Error(`Saldo paper tidak cukup. Cash tersedia ${formatUsd(account.balance, 2)}.`);
    if (desired < 5) throw new Error("Nominal paper buy minimal 5 USDT.");

    const result = await analyzeAsset(item, mode);
    const price = result.ticker.price;
    const fee = tradeFee(desired);
    const quantity = (desired - fee) / price;
    const old = account.positions[item.asset];
    const cost = desired;
    const newCost = Number(old?.cost || 0) + cost;
    const newQuantity = Number(old?.quantity || 0) + quantity;
    const avgEntry = newCost > 0 && newQuantity > 0 ? newCost / newQuantity : price;

    account.positions[item.asset] = {
        asset: item.asset,
        symbol: item.symbol,
        name: item.name,
        ...
        avgEntry,
        cost: newCost,
        fees: Number(old?.fees || 0) + fee,
        highestPrice: Math.max(Number(old?.highestPrice || 0), price),
        mode,
        openedAt: old?.openedAt || Date.now(),
        updatedAt: Date.now(),
        lastSignal: result.signal.action
    };
    account.balance -= desired;
    account.updatedAt = Date.now();
    appendPaperHistory(jid, {
        type: "BUY",
        asset: item.asset,
        price,
        quantity,
    ...
    return `*PAPER BUY ${item.asset} BERHASIL*\nHarga: ${formatUsd(price, priceDigits(price))}\nNominal: ${formatUsd(desired, 2)} | Fee: ${formatUsd(fee, 2)}\nJumlah: ${formatUnits(quantity)} ${item.asset}\nAvg entry: ${formatUsd(avgEntry, priceDigits(avgEntry))}\nSinyal live: ${result.signal.action} ${result.signal.score}/100\n\nIni simulasi paper trading. Tidak ada order asli dikirim ke Binance.`;
}

async function paperSell(jid, asset) {
    const account = getPaperAccount(jid);
    const item = typeof asset === "string" ? normalizeAsset(asset) : asset;
    if (!item) throw new Error("Koin tidak ada di watchlist.");
    const position = account.positions[item.asset];
    if (!position) throw new Error(`Tidak ada posisi paper ${item.asset}.`);
    ...
    account.balance += net;
    delete account.positions[item.asset];
    account.updatedAt = Date.now();
    appendPaperHistory(jid, {
        type: "SELL",
        asset: item.asset,
    ...
    return `*PAPER SELL ${item.asset} BERHASIL*\nHarga exit: ${formatUsd(price, priceDigits(price))}\nJumlah: ${formatUnits(position.quantity)} ${item.asset}\nNilai bersih: ${formatUsd(net, 2)} | Fee: ${formatUsd(fee, 2)}\nPnL: *${formatSignedUsd(pnl, 2)}* (${formatPct(pnlPct)})\nCash paper: ${formatUsd(account.balance, 2)}\n\nSimulasi selesai; tidak ada order asli.`;
}

async function buildPaperPortfolioMessage(jid) {
    const account = getPaperAccount(jid);
    const positions = Object.values(account.positions || {});
    let positionsValue = 0;
    let unrealized = 0;
    let text = `*PAPER PORTFOLIO*\n`;
    text += `Cash: ${formatUsd(account.balance, 2)}\n`;
    text += `Modal awal: ${formatUsd(account.startingBalance, 2)}\n`;

    if (!positions.length) {
        text += `\nBelum ada posisi paper.\nCoba: *paper beli BTC 1000* atau *strategi BTC*.`;
    } else {
        text += `\n*POSISI AKTIF*\n`;
        for (const position of positions) {
            const ticker = await getTicker(position.symbol, { force: false });
            const currentValue = position.quantity * ticker.price;
            const pnl = currentValue - position.cost;
            const pnlPct = position.cost > 0 ? (pnl / position.cost) * 100 : 0;
            positionsValue += currentValue;
            unrealized += pnl;
            text += `${position.asset}: ${formatUnits(position.quantity)} | ${formatPct(pnlPct)} (${formatSignedUsd(pnl, 2)}) | ${formatUsd(ticker.price, priceDigits(ticker.price))}\n`;
        }
    }

    const equity = account.balance + positionsValue;
    const totalPnl = equity - account.startingBalance;
    const totalPnlPct = account.startingBalance > 0 ? (totalPnl / account.startingBalance) * 100 : 0;
    text += `\nEquity: *${formatUsd(equity, 2)}*\n`;
    text += `Total PnL: *${formatSignedUsd(totalPnl, 2)}* (${formatPct(totalPnlPct)})\n`;
    text += `Unrealized: ${formatSignedUsd(unrealized, 2)}\n`;
    text += `Update: ${nowText()}\n`;
    text += `\nPerintah: *paper beli BTC 1000*, *paper jual BTC*, *paper history*, *paper reset 10000*.`;
    return text.trim();
}

function buildPaperHistory(jid) {
    const history = (state.paperHistory[jid] || []).slice(-10).reverse();
    if (!history.length) return "Riwayat paper trading masih kosong.";
    let text = `*PAPER HISTORY*\n`;
    for (const event of history) {
        const pnlText = event.type === "SELL" ? ` | PnL ${formatSignedUsd(event.pnl, 2)} (${formatPct(event.pnlPct)})` : "";
        text += `${event.type} ${event.asset} | ${formatUsd(event.price, priceDigits(event.price))} | ${formatUsd(event.value, 2)}${pnlText}\n`;
    }
    return text.trim();
}
...
async function buildMarketDashboard(jid) {
    ...
    const positions = getUserPositions(jid, false);
    const paperAccount = state.paperAccounts[jid] || null;
...
`;
    ...
    if (paperAccount) {
        const paperPositions = Object.values(paperAccount.positions || {});
        let paperValue = 0;
        for (const position of paperPositions) {
            const row = ready.find(item => item.asset === position.asset);
            paperValue += Number(position.quantity || 0) * Number(row?.ticker.price || position.avgEntry || 0);
        }
        const paperEquity = Number(paperAccount.balance || 0) + paperValue;
        const paperPnlPct = paperAccount.startingBalance > 0
            ? ((paperEquity - paperAccount.startingBalance) / paperAccount.startingBalance) * 100
            : 0;
        text += `\n*PAPER LAB* â€¢ ${paperPositions.length} posisi\n`;
        text += `Equity ${formatUsd(paperEquity, 2)} (${formatPct(paperPnlPct)}) â€¢ Cash ${formatUsd(paperAccount.balance, 2)}\n`;
    }

    text += `\n*QUICK ACTIONS*\n`;
    text += `analisa BTC • strategi BTC • backtest BTC trend • paper`;
    return text;
}
...
function formatPaperAiContext(jid) {
    const account = state.paperAccounts[jid];
    if (!account) return "";
    const positions = Object.values(account.positions || {});
    const positionText = positions.length
        ? positions.map(position => `${position.asset}: qty ${position.quantity}, avg ${position.avgEntry}, cost ${position.cost}, mode ${position.mode}`).join("\n")
        : "Tidak ada posisi paper aktif.";
    return `PAPER PORTFOLIO PENGGUNA
Cash: ${account.balance} USDT
Modal awal: ${account.startingBalance} USDT
Posisi:
${positionText}`;
}
...
async function buildAiMarketContext(question, mode, jid = "") {
    ...
    const hasCryptoSubject = Boolean(asset) || /\b(crypto|kripto|coin|koin|altcoin|bitcoin|ethereum|bnb|binance|paxg|xaut|usdt|market|pasar|paper|backtest|strategi)\b/i.test(lower);
    const asksMarket = hasCryptoSubject && /\b(harga|sekarang|hari ini|terbaru|market|pasar|trading|trader|entry|sell|jual|buy|beli|dibeli|sinyal|analisa|analisis|risiko|portofolio|paper|backtest|strategi|simulasi)\b/i.test(lower);
    ...
    const paperContext = jid ? formatPaperAiContext(jid) : "";
    if (paperContext && /\b(paper|simulasi|portfolio|portofolio|posisi|risiko|pnl|profit|loss)\b/i.test(lower)) {
...
async function buildAutoMarketReport(mode = DEFAULT_MODE, options = {}) {
    ...
    for (const row of analyses) {
        ...
        const topStrategy = [...(row.strategies || [])].sort((a, b) => b.score - a.score)[0];
        if (topStrategy) {
            text += `\n   Strategy: ${topStrategy.name} - ${strategyActionLabel(topStrategy.action)} ${topStrategy.score}/100`;
        }
    }
...
*STRATEGY LAB*
• strategi BTC
• strategi ETH investor
• backtest BTC trend
• backtest ETH breakout investor
    ...
    const server = http.createServer((req, res) => {
        if (req.url === "/" || req.url === "/health") {
            ...
            res.end(JSON.stringify({
                ...
                paper_accounts: Object.keys(state.paperAccounts || {}).length,
                paper_open_positions: Object.values(state.paperAccounts || {}).reduce(
                    (total, account) => total + Object.keys(account.positions || {}).length,
                    0
                ),
                strategies: STRATEGIES.map(strategy => strategy.id),
                backtest_candle_limit: BACKTEST_CANDLE_LIMIT,
                paper_trade_fee_bps: PAPER_TRADE_FEE_BPS,
                ...
                whatsapp_connection: whatsappConnectionState,
                whatsapp_self_chat_enabled: ALLOW_SELF_CHAT,
                last_incoming_message: lastIncomingMessageAt ? timeText(lastIncomingMessageAt) : null,
                last_processed_message: lastProcessedMessageAt ? timeText(lastProcessedMessageAt) : null,
                last_outgoing_message: lastOutgoingMessageAt ? timeText(lastOutgoingMessageAt) : null,
                time: nowText()
            }));
            return;
        }
...
async function handleMessage(sock, msg, upsertType = "notify") {
    ...
    if (msg.key.fromMe) {
        ...
        if (wasSentByBot(msg, text)) return;
        if (!isOwnerSelfChat(msg)) {
            if (WA_MESSAGE_DEBUG) console.log(`Pesan fromMe non-self diabaikan: ${from}`);
            return;
        }
    }
    ...
    if (WA_MESSAGE_DEBUG) {
        console.log(`Pesan diproses dari ${from}${msg.key.fromMe ? " (self-chat)" : ""}: ${pesan.slice(0, 80)}`);
    }
    ...
    try {
        ...
        if (/^(status|cek status)$/i.test(lower)) {
            ...
            const paperAccount = state.paperAccounts[from] || null;
            const paperPositions = Object.keys(paperAccount?.positions || {}).length;
            return sock.sendMessage(from, {
                text: `STATUS BOT
Koneksi WhatsApp: ${whatsappConnectionState}
Alert nomor ini: ${active}
Mode: ${subscriberMode(from).toUpperCase()}
Posisi dipantau: ${openPositions}
Paper trading: ${paperAccount ? `${paperPositions} posisi, cash ${formatUsd(paperAccount.balance, 2)}` : "belum dimulai"}
Strategi aktif: ${STRATEGIES.map(strategy => strategy.id).join(", ")}
Backtest candle limit: ${BACKTEST_CANDLE_LIMIT}
Self-chat: ${ALLOW_SELF_CHAT ? "AKTIF" : "OFF"}
...
Update: ${nowText()}`
        ...
        if (/^(strategi|strategy|strategies|lab|strategy lab)\b/i.test(lower)) {
            const asset = normalizeAsset(pesan);
            if (!asset) {
                return sock.sendMessage(from, { text: "Sebutkan koinnya. Contoh: *strategi BTC* atau *strategi ETH investor*." });
            }
            const mode = /\binvestor\b/i.test(lower) ? "investor" : /\btrader\b/i.test(lower) ? "trader" : subscriberMode(from);
            return sock.sendMessage(from, { text: await buildStrategyBoard(asset, mode) });
        }

        if (/^(backtest|uji strategi|test strategi|bt)\b/i.test(lower)) {
            const asset = normalizeAsset(pesan);
            if (!asset) {
                return sock.sendMessage(from, { text: "Sebutkan koin dan strategi.\nContoh: *backtest BTC trend* atau *backtest ETH breakout investor*." });
            }
            const mode = /\binvestor\b/i.test(lower) ? "investor" : /\btrader\b/i.test(lower) ? "trader" : subscriberMode(from);
            const strategyId = normalizeStrategyId(pesan);
            return sock.sendMessage(from, { text: buildBacktestMessage(await runBacktest(asset, strategyId, mode)) });
        }

        if (/^(paper|simulasi|paper trading)\b/i.test(lower)) {
            const numbers = (pesan.match(/\d+(?:[.,]\d+)?/g) || []).map(parseNumber).filter(Boolean);
            const mode = /\binvestor\b/i.test(lower) ? "investor" : /\btrader\b/i.test(lower) ? "trader" : subscriberMode(from);

            if (/^(paper|simulasi)\s+(history|riwayat|jurnal)$/i.test(lower)) {
                return sock.sendMessage(from, { text: buildPaperHistory(from) });
            }

            if (/^(paper|simulasi)\s+(mulai|start|reset|ulang)\b/i.test(lower)) {
                const account = resetPaperAccount(from, numbers[0] || DEFAULT_PAPER_BALANCE);
                return sock.sendMessage(from, {
                    text: `Paper trading direset.\nModal virtual: ${formatUsd(account.startingBalance, 2)}\n\nCoba: *paper beli BTC 1000* atau *strategi BTC*.\nBot tetap tidak mengeksekusi order asli.`
                });
            }

            if (/^(paper|simulasi)\s+(beli|buy|entry)\b/i.test(lower)) {
                const asset = normalizeAsset(pesan);
                if (!asset) {
                    return sock.sendMessage(from, { text: "Sebutkan koinnya.\nContoh: *paper beli BTC 1000*." });
                }
                return sock.sendMessage(from, { text: await paperBuy(from, asset, numbers[0] || null, mode) });
            }

            if (/^(paper|simulasi)\s+(jual|sell|tutup|close|exit)\b/i.test(lower)) {
                const asset = normalizeAsset(pesan);
                if (!asset) {
                    return sock.sendMessage(from, { text: "Sebutkan koinnya.\nContoh: *paper jual BTC*." });
                }
                return sock.sendMessage(from, { text: await paperSell(from, asset) });
            }

            return sock.sendMessage(from, { text: await buildPaperPortfolioMessage(from) });
        }
...
async function startBot() {
    ...
    try {
        ...
        console.log(`Folder session WhatsApp: ${WHATSAPP_AUTH_DIR}`);
        ...
        const sock = makeWASocket({
            version,
            auth: authState,
            logger: pino({ level: WA_LOG_LEVEL }),
        ...
        sock.ev.on("messages.upsert", async ({ messages, type }) => {
            ...
            if (WA_MESSAGE_DEBUG) console.log(`messages.upsert diterima: type=${type || "-"}, jumlah=${messages?.length || 0}`);
            for (const msg of messages || []) {
                try {
                    await handleMessage(sock, msg, type);
                } catch (err) {
                    if (isClosedSignalSessionError(err)) {
                        console.log("Pesan lama dari sesi Signal tertutup diabaikan; koneksi bot tetap dipertahankan.");
                        continue;
                    }
                    console.error(`Error memproses pesan ${msg?.key?.id || "-"}:`, err.message || err);
                }
            }
        });
...
process.on("uncaughtException", err => {
    if (isClosedSignalSessionError(err)) {
        console.log("Peringatan Signal: pesan dari sesi tertutup diabaikan tanpa reconnect.");
        return;
    }
