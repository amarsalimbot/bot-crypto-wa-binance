...
const BINANCE_API_BASE = String(process.env.BINANCE_API_BASE || "https://api.binance.com").replace(/\/+$/, "");
...
const BINANCE_ERROR_COOLDOWN_SECONDS = Math.max(10, Number(process.env.BINANCE_ERROR_COOLDOWN_SECONDS || 60));
...
const binanceUnsupportedSymbols = new Set();
...
function isBinanceSymbolError(err) {
    const message = String(err?.message || "").toLowerCase();
    return message.includes("-1121") || message.includes("invalid symbol");
}
...
function markBinanceUnavailable(err, symbol = "") {
    if (isBinanceSymbolError(err)) {
        if (symbol) binanceUnsupportedSymbols.add(symbol);
        console.log(`Pair ${symbol || "unknown"} tidak tersedia di Binance. Pair ini memakai CoinGecko fallback.`);
        return;
    }
    const restricted = isBinanceRestrictedError(err);
    const cooldownMs = restricted
        ? BINANCE_RESTRICTED_COOLDOWN_MINUTES * 60 * 1000
        : BINANCE_ERROR_COOLDOWN_SECONDS * 1000;
    const baru = Date.now() + cooldownMs;
    if (baru > binanceBlockedUntil) {
        binanceBlockedUntil = baru;
        const duration = restricted
            ? `${BINANCE_RESTRICTED_COOLDOWN_MINUTES} menit karena batas lokasi/IP`
            : `${BINANCE_ERROR_COOLDOWN_SECONDS} detik karena error sementara`;
        console.log(`Binance tidak tersedia. CoinGecko fallback dipakai selama ${duration}.`);
    }
}
...
function bolehPakaiBinance(symbol = "") {
    if (symbol && binanceUnsupportedSymbols.has(symbol)) return false;
    return Date.now() >= binanceBlockedUntil;
}

function marketProviderText() {
    return "Binance realtime utama -> CoinGecko fallback";
}

function coinGeckoSourceText() {
...
async function fetchBinanceJson(pathname, params = {}, timeoutMs = 15000) {
    const url = new URL(`${BINANCE_API_BASE}${pathname}`);
    ...
    try {
        ...
        if (!response.ok) {
            ...
            throw new Error(`Binance HTTP ${response.status} ${response.statusText} ${body.slice(0, 500)}`.trim());
        }
    ...
    if (bolehPakaiBinance(symbol)) {
        try {
            ...
            if (!Number.isFinite(ticker.price) || ticker.price <= 0) {
                throw new Error(`Binance mengembalikan harga tidak valid untuk ${symbol}`);
            }
            return setCache(key, ticker);
        } catch (err) {
            markBinanceUnavailable(err, symbol);
            console.log(`Binance tidak tersedia untuk ${symbol}; memakai CoinGecko fallback.`);
        }
    }
    ...
    if (bolehPakaiBinance(symbol)) {
        try {
            ...
            if (!Array.isArray(rows) || rows.length < 30) {
                throw new Error(`Binance candle ${symbol} ${interval} kurang data`);
            }
            ...
            markBinanceUnavailable(err, symbol);
            console.log(`Binance candle tidak tersedia untuk ${symbol} ${interval}; memakai CoinGecko/cache fallback.`);
        }
    }
...
async function buildAutoMarketReport(mode = DEFAULT_MODE, options = {}) {
    ...
    text += `📡 Prioritas: ${marketProviderText()} | sumber aktual tertulis per koin\n\n`;
...
- harga dan candle Binance realtime sebagai sumber utama
- CoinGecko otomatis dipakai hanya saat Binance error atau pair tidak tersedia
...
async function handleMessage(sock, msg) {
    ...
    try {
        ...
        if (/^(refresh|refresh data|update data|muat ulang)$/i.test(lower)) {
            ...
            return sock.sendMessage(from, { text: "🔄 Cache dan cooldown market sudah dikosongkan. Request berikutnya akan mencoba Binance realtime lebih dulu." });
        }
...
Prioritas market: ${marketProviderText()}
...
Binance API key: ${BINANCE_API_KEY ? "aktif (opsional untuk endpoint publik)" : "tidak diisi, public API tetap aktif"}
Pair tanpa market Binance: ${binanceUnsupportedSymbols.size ? [...binanceUnsupportedSymbols].join(", ") : "-"}
