...
const AI_QUOTA_COOLDOWN_MS = Math.max(AI_PROVIDER_COOLDOWN_MS, Math.max(5, Number(process.env.AI_QUOTA_COOLDOWN_MINUTES || 60)) * 60_000);
...
const BINANCE_PRIVATE_RESTRICTED_COOLDOWN_MINUTES = Math.max(30, Number(process.env.BINANCE_PRIVATE_RESTRICTED_COOLDOWN_MINUTES || 1440));
...
let binancePrivateBlockedUntil = 0;
...
function hasReadyAiProvider() {
    return Boolean(
        (OPENAI_API_KEY && Date.now() >= openAiBlockedUntil) ||
        (geminiAi && Date.now() >= geminiBlockedUntil)
    );
}
...
function aiCooldownMsForError(err) {
    const message = String(err?.message || err || "").toLowerCase();
    const retry = message.match(/try again in\s+(\d+)m(?:(\d+)s)?/) || message.match(/retry after\s+(\d+)m(?:(\d+)s)?/);
    if (retry) {
        return Math.max(AI_PROVIDER_COOLDOWN_MS, (Number(retry[1] || 0) * 60 + Number(retry[2] || 0) + 30) * 1000);
    }
    const seconds = message.match(/try again in\s+(\d+)s/) || message.match(/retry after\s+(\d+)s/);
    if (seconds) return Math.max(AI_PROVIDER_COOLDOWN_MS, (Number(seconds[1]) + 30) * 1000);
    return isAiQuotaError(err) ? AI_QUOTA_COOLDOWN_MS : AI_PROVIDER_COOLDOWN_MS;
}

function buildAiFailureMessage(err) {
    if (!hasAiProvider()) {
        return "AI belum aktif. Isi OPENAI_API_KEY atau GEMINI_API_KEY jika ingin memakai fitur chat AI.";
    }
    const quota = isAiQuotaError(err);
    const detail = pendekkanError(err?.message || err);
    return `${quota ? "Kuota AI sedang habis / rate limit." : "AI sedang gagal merespons."}
ChatGPT/OpenAI: ${OPENAI_API_KEY ? cooldownText(openAiBlockedUntil) : "belum diisi"}
Gemini: ${geminiAi ? cooldownText(geminiBlockedUntil) : "belum diisi"}

Fitur harga, analisa teknikal, market pressure, dan trading tetap bisa dipakai.
Untuk hemat kuota di Railway, set AI_AUTO_CHAT=false.
...
Cooldown quota AI: ${Math.round(AI_QUOTA_COOLDOWN_MS / 60000)} menit`;
}
...
async function generateAiText({ instructions, prompt, purpose = "chat" }) {
    ...
    if (OPENAI_API_KEY && Date.now() >= openAiBlockedUntil) {
        try {
            ...
            openAiBlockedUntil = Date.now() + aiCooldownMsForError(err);
    ...
    if (geminiAi && Date.now() >= geminiBlockedUntil) {
        try {
            ...
            geminiBlockedUntil = Date.now() + aiCooldownMsForError(err);
...
function makeBinancePrivateRestrictedError() {
    const err = new Error(
        "Live trading Binance diblokir karena IP/lokasi server Railway mendapat HTTP 451 restricted location. " +
        "CoinGecko tidak dapat menggantikan saldo atau order Binance. Gunakan TRADING_MODE=paper, atau jalankan service dari wilayah yang memenuhi ketentuan Binance jika akun Anda memang memenuhi syarat."
    );
    err.code = "BINANCE_PRIVATE_RESTRICTED";
    return err;
}

function markBinancePrivateRestricted(err) {
    if (!isBinanceRestrictedError(err) && err?.code !== "BINANCE_PRIVATE_RESTRICTED") return;
    const baru = Date.now() + BINANCE_PRIVATE_RESTRICTED_COOLDOWN_MINUTES * 60 * 1000;
    if (baru > binancePrivateBlockedUntil) {
        binancePrivateBlockedUntil = baru;
        console.log(`Binance private API dibatasi lokasi/IP. Live trading dikunci selama ${BINANCE_PRIVATE_RESTRICTED_COOLDOWN_MINUTES} menit.`);
    }
}

function ensureBinancePrivateAvailable() {
    if (Date.now() < binancePrivateBlockedUntil) throw makeBinancePrivateRestrictedError();
}
...
async function fetchBinancePrivate(pathname, params = {}, method = "GET", timeoutMs = 15000) {
    ...
    try {
        ...
        if (!response.ok) {
            const err = new Error(`Binance private HTTP ${response.status}: ${data?.msg || raw.slice(0, 240)}`);
            err.httpStatus = response.status;
            if (response.status === 451 || isBinanceRestrictedError(err)) {
                markBinancePrivateRestricted(err);
                throw makeBinancePrivateRestrictedError();
            }
            throw err;
        }
        return data;
...
function tradingModeLabel() {
    if (TRADING_MODE === "live" && ENABLE_LIVE_TRADING && Date.now() < binancePrivateBlockedUntil) return "LIVE - PRIVATE API RESTRICTED 451";
...
function buildProcessErrorMessage(err) {
    if (err?.code === "BINANCE_PRIVATE_RESTRICTED" || isBinanceRestrictedError(err)) {
        return `LIVE TRADING BINANCE DIBLOKIR
Server Railway mendapat HTTP 451 restricted location dari Binance.

Harga publik masih bisa memakai CoinGecko fallback, tetapi balance dan order live tidak bisa dijalankan melalui CoinGecko.

Solusi aman:
1. Ubah Railway variable TRADING_MODE=paper dan ENABLE_LIVE_TRADING=false; atau
2. Gunakan server di wilayah yang memenuhi ketentuan Binance, hanya jika akun Anda memang memenuhi syarat.

API key baru tidak akan memperbaiki blokir lokasi/IP ini.
Status private API: ${cooldownText(binancePrivateBlockedUntil)}`;
    }
    return `Gagal memproses permintaan: ${pendekkanError(err?.message || "unknown error")}. Coba lagi beberapa saat lagi.`;
}
...
${order.sellPercent ? `Jual: ${order.sellPercent}% saldo ${validation.info.baseAsset}\n` : ""}${order.sellAll ? `Jual: semua saldo ${validation.info.baseAsset}\n` : ""}Estimasi saldo terpakai: ${validation.quantity} ${validation.info.baseAsset}
...
Binance private API/live: ${cooldownText(binancePrivateBlockedUntil)}
...
async function sendAiAnswer(sock, jid, question) {
    ...
    try {
        ...
        console.log(`AI answer gagal untuk ${jid}: ${pendekkanError(err.message || err)}`);
        return await sock.sendMessage(jid, { text: buildAiFailureMessage(err) });
    } finally {
        if (typeof sock.sendPresenceUpdate === "function") {
...
function parsePercent(value) {
    ...
    if (!Number.isFinite(percent) || percent <= 0 || percent > 100) {
        throw new Error("Persen jual harus angka > 0 sampai 100. Contoh: 25 atau 25%.");
    }
    return percent;
}

async function buildSellPercentOrder({ symbolArg, percentArg, type = "MARKET", priceArg = null }) {
    ensureTradingAllowed();
    const symbol = normalizeTradingSymbol(symbolArg);
    const percent = parsePercent(percentArg);
    const info = await getExchangeInfo(symbol);
    if (info.status !== "TRADING") throw new Error(`Symbol ${symbol} status Binance: ${info.status}`);
    const price = type === "LIMIT" ? parsePositiveNumber(priceArg, "Harga limit") : (await getBinanceTicker(symbol)).price;
    const baseBalance = await getTradableBaseBalance(info);
    const rawQuantity = baseBalance * (percent / 100);
    const quantity = Number(formatStepNumber(rawQuantity, info.stepSize));
    if (!quantity || quantity < info.minQty) {
        throw new Error(`Saldo ${info.baseAsset} tidak cukup untuk jual ${percent}%. Saldo: ${baseBalance}`);
    }
    const quoteAmount = quantity * price;
    if (quoteAmount < info.minNotional) {
        throw new Error(`Nilai jual ${formatUsd(quoteAmount, 2)} di bawah minimum Binance (${formatUsd(info.minNotional, 2)}).`);
    }
...
async function prepareSellPercentOrder(jid, { symbolArg, percentArg, type = "MARKET", priceArg = null }) {
    const order = await buildSellPercentOrder({ symbolArg, percentArg, type, priceArg });
    const validation = await validateOrderRequest(order);
    const preview = buildOrderPreview(order, validation);
...
Catatan risiko: perintah ini menjual ${order.sellAll ? "semua" : `${order.sellPercent}%`} saldo ${validation.info.baseAsset} yang tersedia saat preview dibuat.`;
    }
...
function canonicalTradingCommand(rawCommand, parts) {
    const raw = String(rawCommand || "").replace(/^[\/!.]+/, "").toLowerCase();
    const aliases = {
        saldo: "balance",
        beli_market: "buy_market",
        beli: "buy_market",
        buy: "buy_market",
        jual_market: "sell_market",
        jual: "sell_market",
        sell: "sell_market",
        beli_limit: "buy_limit",
        jual_limit: "sell_limit",
        jual_semua: "sell_all",
        sell_semua: "sell_all",
        jual_all: "sell_all",
        jual_persen: "sell_percent",
        sell_persen: "sell_percent",
        sell_percent: "sell_percent",
        jual_percent: "sell_percent",
        jual_persen_limit: "sell_percent_limit",
        sell_persen_limit: "sell_percent_limit",
        sell_percent_limit: "sell_percent_limit",
        jual_percent_limit: "sell_percent_limit",
        order_aktif: "open_orders",
        batal_order: "cancel_order",
        batalkan_order: "cancel_order",
        tekanan_beli: "demand",
        tekanan_jual: "sell_pressure",
        posisi: "positions",
        laporan_trading: "report",
        konfirmasi: "confirm"
    };

    if (["jual", "sell"].includes(raw)) {
        const firstArg = String(parts[1] || "").toLowerCase();
        const secondArg = String(parts[2] || "").toLowerCase();
        if (["semua", "all", "100%"].includes(firstArg) || ["semua", "all"].includes(secondArg)) return "sell_all";
        if (String(parts[1] || "").includes("%") || String(parts[2] || "").includes("%")) return "sell_percent";
    }

    return aliases[raw] || raw;
}

function parsePercentCommandArgs(parts) {
    if (String(parts[1] || "").includes("%")) {
        return { percentArg: parts[1], symbolArg: parts[2], priceArg: parts[3] };
    }
    return { symbolArg: parts[1], percentArg: parts[2], priceArg: parts[3] };
}

function parseSellAllSymbolArg(parts) {
    const firstArg = String(parts[1] || "").toLowerCase();
    if (["semua", "all", "100%"].includes(firstArg)) return parts[2];
    return parts[1];
}
...
async function handleTradingCommand(jid, pesan) {
    ...
    const command = canonicalTradingCommand(parts[0], parts);
    const known = new Set([
        "bot_on", "bot_off", "auto_status", "balance", "price",
        "buy_market", "sell_market", "buy_limit", "sell_limit", "sell_all", "sell_percent", "sell_percent_limit",
    ...
    if (command === "sell_market") {
        if (String(parts[2] || "").includes("%")) {
            return await prepareSellPercentOrder(jid, { symbolArg: parts[1], percentArg: parts[2], type: "MARKET" });
        }
    ...
    if (command === "sell_limit") {
        if (String(parts[2] || "").includes("%")) {
            return await prepareSellPercentOrder(jid, { symbolArg: parts[1], percentArg: parts[2], type: "LIMIT", priceArg: parts[3] });
        }
        return await prepareManualOrder(jid, { side: "SELL", type: "LIMIT", symbolArg: parts[1], quoteArg: parts[2], priceArg: parts[3] });
    }
    if (command === "sell_percent") {
        const args = parsePercentCommandArgs(parts);
        return await prepareSellPercentOrder(jid, { ...args, type: "MARKET" });
    }
    if (command === "sell_percent_limit") {
        const args = parsePercentCommandArgs(parts);
        return await prepareSellPercentOrder(jid, { ...args, type: "LIMIT" });
    }
    if (command === "sell_all") return await prepareSellPercentOrder(jid, { symbolArg: parseSellAllSymbolArg(parts), percentArg: 100, type: "MARKET" });
    ...
    const server = http.createServer((req, res) => {
        if (req.url === "/" || req.url === "/health") {
            ...
            res.end(JSON.stringify({
                ...
                binance_private_api_cooldown: cooldownText(binancePrivateBlockedUntil),
...
async function handleMessage(sock, msg) {
    ...
    try {
        ...
        if (/^(ai|chatgpt|chat|tanya)(\s|$)/i.test(pesan)) {
            ...
            return await sendAiAnswer(sock, from, question);
        }
...
Update: ${nowText()}`
        ...
        if (AI_AUTO_CHAT && hasReadyAiProvider() && !String(from).endsWith("@g.us")) {
            return await sendAiAnswer(sock, from, pesan);
        }
...
Ketik menu untuk daftar lengkap.`
        ...
        return sock.sendMessage(from, {
            text: buildProcessErrorMessage(err)
        });
    }
}
