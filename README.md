...
BINANCE_PRIVATE_RESTRICTED_COOLDOWN_MINUTES=1440
...
- `AI_QUOTA_COOLDOWN_MINUTES` cooldown lebih panjang saat OpenAI/Gemini membalas `429`, rate limit, atau quota exceeded.
...
- `BINANCE_PRIVATE_RESTRICTED_COOLDOWN_MINUTES` mengunci percobaan saldo/order live setelah private API Binance membalas `451`. Default 1440 menit.
...
Jika kuota AI cepat habis, set `AI_AUTO_CHAT=false` di Railway. Dengan begitu AI hanya dipakai saat pesan diawali `ai`, `chatgpt`, `chat`, atau `tanya`, sementara fitur harga, analisa, market pressure, dan trading tetap berjalan.
...
Perintah manual bisa pakai slash atau tanpa slash. Contoh jual berdasarkan saldo koin:

```text
/sell_market BTCUSDT 25%
/sell_percent BTCUSDT 25
/sell_limit BTCUSDT 50% 65000
/sell_all BTCUSDT
jual BTCUSDT 25%
jual semua BTCUSDT
...
Jika memakai persen, bot menghitung dari saldo aset yang tersedia. Contoh `jual BTCUSDT 25%` berarti menjual 25% saldo BTC, bukan 25 USDT.
...
Jika Binance private API membalas `HTTP 451 restricted location`, server Railway Anda tidak dapat menjalankan balance/order live Binance dari lokasi/IP tersebut. API key baru dan CoinGecko tidak dapat memperbaiki private order. Gunakan mode aman berikut:
...
Alternatifnya, gunakan server di wilayah yang memenuhi ketentuan Binance hanya jika akun Anda memang memenuhi syarat. Jangan mencoba melewati pembatasan wilayah Binance.
