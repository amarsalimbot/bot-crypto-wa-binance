...
- Strategy Lab multi-strategi: Composite, Trend Rider, Smart Pullback, dan Volume Breakout.
- Backtesting historis long-only berbasis candle dengan win rate, profit factor, max drawdown, fee simulasi, dan sinyal terakhir.
- Paper trading virtual per nomor WhatsApp: cash, posisi, average entry, PnL, history, dan reset modal.
...
PAPER_TRADE_FEE_BPS=10
BACKTEST_CANDLE_LIMIT=420
...
- `ALLOW_SELF_CHAT=true` membuat bot dapat merespons perintah dari chat "Kirim pesan ke diri sendiri" tanpa membalas pesan bot sendiri.
- `WA_MESSAGE_DEBUG=true` menampilkan log penerimaan dan pemrosesan pesan untuk diagnosis. Gunakan `false` setelah masalah selesai.
...
- `DEFAULT_PAPER_BALANCE` adalah modal awal virtual untuk paper trading per chat.
- `PAPER_TRADE_FEE_BPS` adalah fee simulasi dalam basis point. `10` berarti 0,10%.
- `BACKTEST_CANDLE_LIMIT` membatasi jumlah candle untuk backtest, minimal 120 dan maksimal 1000.
...
## Strategy Lab, Backtesting, dan Paper Trading

Fitur ini membuat bot lebih serius untuk latihan trading, tetapi tetap aman: bot hanya membuat sinyal, jurnal, simulasi, dan paper portfolio. Bot tidak meminta API trading Binance dan tidak pernah mengirim order asli.
...
- `composite` - gabungan Trend Rider, Smart Pullback, dan Volume Breakout.
- `trend` - mengikuti EMA dan MACD.
- `pullback` - mencari pantulan sehat dalam tren naik.
- `breakout` - mencari tembus resistance dengan konfirmasi volume.
...
Backtest memakai candle historis dari provider market yang tersedia, modal awal virtual dari `DEFAULT_PAPER_BALANCE`, fee simulasi dari `PAPER_TRADE_FEE_BPS`, dan batas candle dari `BACKTEST_CANDLE_LIMIT`.

Contoh paper trading:
...
Data paper trading disimpan di `data/crypto-bot-state.json` pada field `paperAccounts` dan `paperHistory`, tetap sederhana seperti state JSON bot yang sudah ada.
...
## Bot Online tetapi Tidak Merespons Chat

Pastikan variable berikut tersedia:

```text
ALLOW_SELF_CHAT=true
WA_MESSAGE_DEBUG=true
```

Redeploy lalu kirim `menu`. Pada Railway Logs harus muncul:

```text
Bot Analisa Crypto terhubung dan online.
messages.upsert diterima: type=notify, jumlah=1
Pesan diproses dari ...
```

Jika `messages.upsert` tidak muncul, session WhatsApp belum menerima pesan: cek Railway Volume `/app/session`, pastikan hanya satu replica aktif, lalu tautkan ulang perangkat. Jika `messages.upsert` muncul tetapi `Pesan diproses` tidak muncul, periksa bahwa `WHATSAPP_PHONE_NUMBER` berisi nomor bot yang benar dalam format internasional tanpa `+`.
